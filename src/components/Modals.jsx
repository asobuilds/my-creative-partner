...
export function AuthModal({ isOpen, onClose, onAuthSuccess }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', backdropFilter: 'blur(16px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: '16px' }}>
      <div style={{ background: 'rgba(15, 23, 42, 0.85)', border: '1px solid rgba(0, 240, 255, 0.3)', borderRadius: '24px', padding: '32px', width: '100%', maxWidth: '420px', position: 'relative' }}>
        ...
        <h3 style={{ fontSize: '1.4rem', margin: '0 0 6px 0' }}>
          {mode === 'login' ? 'Sign In' : 'Create Account'}
        </h3>
        <p style={{ fontSize: '0.82rem', color: '#9ca3af', marginBottom: '24px' }}>Access your spatial workspace and saved models.</p>
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', backdropFilter: 'blur(16px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: '16px' }}>
      <div style={{ background: '#0f172a', border: '1px solid rgba(0,240,255,0.3)', borderRadius: '20px', padding: '32px', width: '100%', maxWidth: '400px', position: 'relative' }}>
        ...
        <button onClick={onClose} style={{ background: 'rgba(255,255,255,0.1)', border: 'none', color: '#fff', padding: '10px 20px', borderRadius: '10px', cursor: 'pointer', marginTop: '20px' }}>Close</button>
      </div>
    </div>
  );
}

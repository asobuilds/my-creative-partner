...
export default function ChildMode() {
  ...
  const onShare = async () => {
    ...
    try {
      ...
      const res2 = await fetch(API + '/api/story/render', {
        ...
        body: JSON.stringify({ imageUrl: dataUrl, title: '9jaWonderPal', subtitle: promptText.slice(0, 80), durationSec: 6 }),
      });
  ...
  const onAnimate = async () => {
    ...
    try {
      ...
      else console.error('animate error', j.error);
    } catch (e) { console.error('animate failed', e); }
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%', background: '#0a0812', overflow: 'hidden' }}>
      ...
      {showReference && refData && (
        <ReferencePanel data={refData} loading={refLoading} onDismiss={() => { dismissRef(); setShowReference(false); }} mood={mood} prompt={lastPrompt} />
      )}

      <div style={{ position: 'absolute', top: 18, left: '50%', transform: 'translateX(-50%)', display: 'flex', alignItems: 'center', gap: 12, padding: '8px 18px', borderRadius: 30, background: 'rgba(10,8,18,0.85)', backdropFilter: 'blur(14px)', border: '1px solid ' + warm + '44', color: warm, fontSize: 13, fontWeight: 700, zIndex: 15, maxWidth: '90vw' }}>
      ...
      {storyUrl && (
        <div style={{ position: 'absolute', top: 76, left: 18, padding: '10px 18px', borderRadius: 14, background: 'rgba(37,211,102,0.2)', border: '1px solid #25D366', color: '#25D366', fontSize: 13, fontWeight: 700, zIndex: 20, display: 'flex', gap: 10, alignItems: 'center' }}>
          ...
          <button onClick={() => setStoryUrl(null)} style={{ background: 'transparent', border: 'none', color: '#22c55e', cursor: 'pointer' }}>×</button>
        </div>
      )}
      {savedToast && (
      ...
      <form
        ...
        style={{ position: 'absolute', left: '50%', transform: 'translateX(-50%)', bottom: 34, zIndex: 15, width: 'min(720px, calc(100% - 32px))', display: 'flex', gap: 10, alignItems: 'center', padding: '10px 10px 10px 22px', background: 'rgba(10,8,18,0.94)', backdropFilter: 'blur(22px)', border: '2px solid ' + (listening ? '#ffb700' : warm + '77'), borderRadius: 40, boxShadow: listening ? '0 0 0 6px rgba(255,183,0,0.15), 0 20px 60px rgba(0,0,0,0.6)' : '0 20px 60px rgba(0,0,0,0.6)', transition: 'all .25s' }}
      >
        <input
          ref={inputRef}
        ...
        <button
          ...
          style={{ width: 64, height: 64, borderRadius: '50%', background: listening ? 'linear-gradient(135deg,#ffb700,#ff6b00)' : 'linear-gradient(135deg,' + warm + ',' + mood.accent + ')', border: 'none', color: listening ? '#fff' : '#000', cursor: supported ? 'pointer' : 'not-allowed', display: 'flex', alignItems: 'center', justifyContent: 'center', animation: listening ? 'micPulse 1.2s ease-in-out infinite' : 'none', boxShadow: listening ? '0 0 24px rgba(255,183,0,0.6)' : 'none', transition: 'all .2s' }}
        >
            ...
            <div key={w.id} style={{ marginBottom: 14, padding: 12, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 14 }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                <button onClick={async () => {
                  try {
                    const API = (import.meta.env.VITE_API_BASE || 'http://localhost:5000').replace(/\/$/, '');
                    const img = w.preview ? (w.preview.startsWith('http') ? w.preview : API + w.preview) : null;
                    if (!img) return;
                    const blob = await (await fetch(img)).blob();
                    const dataUrl = await new Promise((r) => { const fr = new FileReader(); fr.onload = () => r(fr.result); fr.readAsDataURL(blob); });
                    const res = await fetch(API + '/api/story/render', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ imageUrl: dataUrl, title: '9jaWonderPal', subtitle: w.promptSummary.slice(0, 80), durationSec: 6 }) });
                    const j = await res.json();
                    if (j.url) window.open(j.url, '_blank');
                  } catch (e) { console.error(e); }
                }} style={{ padding: '6px 10px', borderRadius: 8, background: 'rgba(37,211,102,0.15)', color: '#25D366', border: '1px solid rgba(37,211,102,0.4)', fontSize: 11, cursor: 'pointer' }}>Share</button>

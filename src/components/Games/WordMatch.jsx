...
import { ArrowLeft, Check, X, RefreshCw, ChevronRight, Flame } from 'lucide-react';
        ...
        <div style={{ maxWidth: 560, margin: '0 auto' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <div style={{ padding: '4px 10px', borderRadius: 8, background: 'rgba(167,139,250,0.2)', color: '#6d28d9', fontSize: 11, fontWeight: 800 }}>Stage {stage}/5</div>
              <div style={{ padding: '6px 14px', borderRadius: 12, background: 'rgba(255,183,0,0.2)', color: '#92400e', fontSize: 14, fontWeight: 800 }}>{score} pts</div>
            </div>
          </div>

          <div style={{ display: 'flex', justifyContent: 'center', gap: 6, marginBottom: 20 }}>
            {queue.map((_, i) => (
              <div key={i} style={{ width: i === roundIndex ? 24 : 8, height: 8, borderRadius: 4, background: i < roundIndex ? '#22c55e' : (i === roundIndex ? '#ffb700' : 'rgba(0,0,0,0.1)'), transition: 'all .3s' }} />
            ))}
          </div>
          ...
          <div style={{ textAlign: 'center', marginBottom: 24 }}>
            ...
            <div style={{ fontSize: 12, color: '#475569', letterSpacing: 1.5, textTransform: 'uppercase', marginBottom: 6 }}>Which word matches?</div>
            <div style={{ fontSize: 32, fontWeight: 800, color: '#0a1f44', letterSpacing: -0.5 }}>{current.meaning}</div>
          </div>
              ...
              return (
                <button key={opt.word} onClick={() => pick(opt)} disabled={!!picked}
                  style={{
                    ...
                    background: showCorrect ? 'rgba(34,197,94,0.2)' : showWrong ? 'rgba(239,68,68,0.2)' : 'rgba(255,255,255,0.85)',
                    border: '2px solid ' + (showCorrect ? '#22c55e' : showWrong ? '#ef4444' : 'rgba(0,0,0,0.08)'),
          ...
          {feedback === 'wrong' && (
            <div style={{ marginTop: 20, textAlign: 'center', fontSize: 15, color: '#dc2626' }}>

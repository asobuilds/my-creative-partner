      ...
      <WonderBackground variant="games">
        <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', padding: 20 }}>
          ...
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', justifyContent: 'center' }}>
            {stage < 5 && (
              <button onClick={() => setStage(stage + 1)} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '14px 28px', borderRadius: 14, background: 'linear-gradient(135deg,#22d3ee,#3b82f6)', color: '#fff', border: 'none', fontSize: 16, fontWeight: 800, cursor: 'pointer' }}>
        ...
        <div style={{ maxWidth: 640, margin: '0 auto' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <div style={{ padding: '4px 10px', borderRadius: 8, background: 'rgba(34,211,238,0.2)', color: '#0369a1', fontSize: 11, fontWeight: 800 }}>Stage {stage}/5</div>
              <div style={{ padding: '6px 14px', borderRadius: 12, background: 'rgba(34,211,238,0.15)', color: '#0369a1', fontSize: 14, fontWeight: 800 }}>{score} pts</div>
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(110px, 1fr))', gap: 12, marginBottom: 24 }}>
            {slots.map((slot) => (
              <div key={slot.key}
                ...
                style={{ padding: 16, borderRadius: 18, background: slot.filled ? slot.color + '22' : 'rgba(255,255,255,0.75)', border: '2px dashed ' + (slot.filled ? slot.color : 'rgba(0,0,0,0.15)'), minHeight: 100, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
          ...
          <div style={{ padding: 18, borderRadius: 18, background: 'rgba(255,255,255,0.7)', border: '1px solid rgba(0,0,0,0.08)' }}>

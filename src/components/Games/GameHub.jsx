...
import { startGame as sStartGame, tap as sTap } from '../../engine/sound';
...
export default function GameHub({ onLaunchGame }) {
  ...
  const launch = (game) => {
    try { sTap(); } catch (e) {}
    setTimeout(() => { try { sStartGame(); } catch (e) {} }, 120);
    setPicked(game.key);
  ...
  return (
    ...
    <div style={{ height: '100%', overflowY: 'auto', paddingTop: 40, paddingBottom: 60 }}>
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '0 20px' }}>
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          ...
          <h1 style={{ fontSize: 30, fontWeight: 800, color: '#0a1f44', margin: '0 0 10px', letterSpacing: -0.5 }}>Games</h1>
          <p style={{ fontSize: 15, color: '#475569', maxWidth: 460, margin: '0 auto', lineHeight: 1.6 }}>
        ...
        {shown.length === 0 && activeChild && (
          <div style={{ padding: 40, textAlign: 'center', color: '#64748b', background: 'rgba(255,255,255,0.7)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 20 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 14 }}>
          {shown.map((g) => (
            <button key={g.key} onClick={() => launch(g)}
              style={{ padding: 24, borderRadius: 20, background: 'rgba(255,255,255,0.7)', border: '1px solid ' + g.color + '44', cursor: 'pointer', textAlign: 'left', transition: 'all .15s', minHeight: 160, display: 'flex', flexDirection: 'column' }}>
              <div style={{ fontSize: 40, marginBottom: 12 }}>{g.emoji}</div>
              <div style={{ fontSize: 16, fontWeight: 800, color: '#0a1f44', marginBottom: 6 }}>{g.name}</div>
              <div style={{ fontSize: 13, color: '#475569', lineHeight: 1.5, flex: 1 }}>{g.desc}</div>

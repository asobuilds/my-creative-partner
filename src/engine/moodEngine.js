...
export function deriveMood(nodes = [], companionText = '') {
  const lowered = (companionText || '').toLowerCase();
  ...
  const moodNode = nodes.find((n) => n.type === 'mood');
...
export function applyMood(key) {
  ...
  useStudioStore.getState().setMood(palette);
  return palette;
}
...
export function startMoodSync() {
  ...
  return useStudioStore.subscribe((state) => {
    const key = deriveMood(state.nodes, state.companion.text);
    if (key !== last) { last = key; applyMood(key); }
  });
}
